...
if uploaded_files:
    ...
    if processed_dfs:
        ...
        if all_data_list:
            ...
            # ==============================================================================
            st.markdown("---")
            st.header("📂 品类维度指标汇总", divider="green")
            
            # 为了不影响其他逻辑，使用拷贝进行计算
            df_cat = df_all.copy()
            
            # 检查原始数据中是否存在“品类”列，避免报错崩溃
            if '品类' in df_cat.columns:
                # 填充空值以防 groupby 遗漏
                df_cat['品类'] = df_cat['品类'].fillna('未分类')
                ...
                # 1. 总体指标汇总
                overall_cat = df_cat.groupby('品类').agg(
                    笔记数=('序号', 'count'),
                    曝光=('曝光', 'sum'),
                    阅读=('观看量', 'sum'),
                    总估算点击=('估算点击数', 'sum'),
                ...
                res_overall = overall_cat[['品类', '笔记数', '曝光', '阅读', '封面点击率', '互动率', '转粉率']].copy()
                
                # 2. 月均指标汇总
                # 先按品类和年月汇总当月指标
                monthly_cat = df_cat.groupby(['品类', '年月']).agg(
                    月笔记数=('序号', 'count'),
                    月曝光=('曝光', 'sum'),
                    月阅读=('观看量', 'sum'),
                    月总估算点击=('估算点击数', 'sum'),
                ...
                # 再按品类计算各月份的算术平均值
                monthly_avg_cat = monthly_cat.groupby('品类').agg(
                    月均笔记数=('月笔记数', 'mean'),
                    月均曝光=('月曝光', 'mean'),
                    月均阅读=('月阅读', 'mean'),
                    月均封面点击率=('月点击率', 'mean'),
                ...
                # 3. 拼接总体与月均，并展示
                final_cat_df = pd.merge(res_overall, monthly_avg_cat, on='品类', how='left')
                
                st.write("**📊 提取所有文件中的品类，按其划分的各项核心表现如下：**")
                st.dataframe(
                ...
                st.warning("⚠️ 在上传的表格中未找到名为【品类】的列，无法生成品类维度的透视分析，请检查您的原始 Excel 结构。")
        ...
        with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
            ...
            exclude_cols = ["账号名", "年月", "月份", "序号", "笔记标题", "首次发布时间", "体裁", "品类"]
