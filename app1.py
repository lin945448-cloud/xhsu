...
if uploaded_files:
    ...
    if processed_dfs:
        ...
        # ==============================================================================
        st.markdown("---")
        st.header("🏆 账号/月份 核心指标对比 & 同比分析", divider="orange")
        ...
        if all_data_list:
            ...
            # ------------------------------------------------------------------
            # B. 📅 核心指标同比透视表 (YoY Pivot Table)
            # ------------------------------------------------------------------
            st.markdown("---")
            st.subheader("📅 核心指标同比透视表 (YoY Analysis)")
            st.info("此表格将数据按【年份】并列展示，方便直接对比同一月份在不同年份的表现（例如：今年10月 vs 去年10月）。")
            ...
            target_account = c_yoy1.selectbox("选择要进行同比分析的账号", all_accounts)
            target_metric_label = c_yoy2.selectbox("选择要对比的指标", list(yoy_metric_map.keys()))
            ...
            if target_account:
                ...
                if not df_target.empty:
                    ...
                    try:
                        ...
                        st.warning("数据不足，无法生成跨年对比表（可能只有一个年份的数据）。")
                else:
                    st.warning("该账号无数据。")
        ...
        with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
            ...
            if 'df_trend' in locals():
                df_trend.to_excel(writer, sheet_name="月度聚合统计(含涨粉)", index=False)

        with placeholder_top.container():
            st.header("汇总Excel下载", divider="rainbow")
            st.download_button("⬇️ 下载汇总Excel报告（含月度涨粉分析）",
