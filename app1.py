...
def analyze_and_display(df, filename):
    ...
    # ---------- 日期解析 ----------
    df["首次发布时间"] = pd.to_datetime(df["首次发布时间"],
    ...
    display_cols = ["序号", "笔记标题", "首次发布时间", "体裁", "曝光", "观看量",
    ...
    # ====================================================
    # 🌟 新增：每个指标单独图，数字稍远、字体大一点
    # ====================================================
    st.subheader("📊 各单项指标趋势图")
    indicator_cols = ["点赞率", "收藏率", "赞藏比", "评论率",
                      "互动率", "有效活跃度", "转粉率"]
    for col in indicator_cols:
        fig, ax = plt.subplots(figsize=(12, 4))
        ax.plot(df["序号"], df[col], color="#66b3ff",
                marker="o", linestyle="-")
        for x, y in zip(df["序号"], df[col]):
            if pd.notna(y):
                if '率' in col:
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            if df_processed is not None:
                sheet_name = ''.join(e for e in uploaded_file.name
                                     if e.isalnum())[:31]
