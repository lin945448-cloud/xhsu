...
import io, os, base64
import numpy as np  # 必须确保导入 numpy
...
def analyze_and_display(df, filename):
    ...
    ax_pie.set_title("图文 vs 视频比例（整体）")
    save_fig_to_html(fig_pie, "图文 vs 视频比例（整体）", html_parts)

    # 👉 按“年月”拆分：每个月输出一组图
    month_list = sorted(df["年月"].unique())
    for month in month_list:
        df_m = df[df["年月"] == month].copy()
        if df_m.empty:
            continue

        # 按时间排序，并为当月重新编号“序号”（月内序号）
        df_m.sort_values(by="首次发布时间", inplace=True)
        df_m.reset_index(drop=True, inplace=True)
        df_m["月内序号"] = df_m.index + 1

        # 为了复用原来的绘图函数，把“月内序号”重命名成“序号”
        df_plot = df_m.rename(columns={"月内序号": "序号"})

        # 月度标题
        html_parts.append(f"<h2>{month} 月度图表</h2>")

        # 1）该月的核心互动指标趋势
        fig_m, ax_m = plot_core_interaction(df_plot)
        save_fig_to_html(fig_m, f"{month} 核心互动指标趋势", html_parts)

        # 2）该月的单指标趋势图
        for col in ["点赞率", "收藏率", "赞藏比", "评论率", "互动率", "有效活跃度", "转粉率"]:
            fig, ax = plt.subplots(figsize=(12, 4))
            plot_lines(ax, f"{month} {col} 趋势图", [col], df_plot)
        ...
        if all_data_list:
            ...
            if selected_accounts:
                            ...
                            if len(sub_data) < 24:
                                for x, y in zip(x_vals, y_vals):
                                    if pd.notna(y):
                                        ...
                                        ax.text(
                                            x, y, txt, ha='center', va='bottom', fontsize=12,
                                            color='black',
                ...
                with st.expander("📋 点击展开：查看月度对比详细数据表（含环比增长率）", expanded=True):
                    ...
                    display_cols = ['账号名', '年月', '涨粉', '涨粉环比', '互动率', '互动率环比',
                                    '封面点击率', '点击率环比', '曝光', '观看量']
        ...
        with pd.ExcelWriter(excel_buffer, engine="openpyxl") as writer:
            ...
            if all_data_list:
                pd.concat(all_data_list, ignore_index=True).to_excel(
                    writer, sheet_name="所有数据明细汇总", index=False
                )
